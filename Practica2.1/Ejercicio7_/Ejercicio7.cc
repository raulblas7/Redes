...
public:
    ...
    void haz_conex(){
        ...
        while (serverAct)
        {
            ...
            int bytesRcv = recv(sd, (void *)buffer, (MESSAGE_MAX_SIZE - 1)*sizeof(char), 0);
...
int main(int argc, char **argv)
{
    ...
    while(true){
        ...
        int clienteSd = accept(sd, &addrClient, &sockLength);
        ...
        MessageThread *mssgThread = new MessageThread(clienteSd);
        std::thread([&mssgThread]() { mssgThread->haz_conex(); delete mssgThread; }).detach();
    }
